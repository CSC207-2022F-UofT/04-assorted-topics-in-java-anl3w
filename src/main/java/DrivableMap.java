...
class DrivableMap {
    ...
    public boolean hasFasterThan (int speed){
        List<String> items = new ArrayList<>(drivable_map.keySet());
        for (int i = 0; i < items.size(); i++){
            if (this.drivable_map.get(items.get(i)).getMaxSpeed() >= speed){
                return true;
            }
        }
        return false;
    ...
    public List<Tradable> getTradable() {
        ...
        for (int i = 0; i < items.size(); i++) {
            Drivable item = this.drivable_map.get(items.get(i));
            if (item instanceof Tradable) {
                trades.add((Tradable) item);
            }
        }
        return trades;
    }
